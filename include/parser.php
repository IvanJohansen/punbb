...
function preparse_bbcode($text, &$errors, $is_signature = false)
{
	...
	if ($forum_config['p_sig_bbcode'] == '1' && $is_signature || $forum_config['p_message_bbcode'] == '1' && !$is_signature)
	{
		...
		$text = preg_replace_callback($pattern_callback, function($matches) {return $replace_callback;}, $text);
...
function preparse_list_tag($content, $type = '*', &$errors)
{
	...
	if (strpos($content,'[list') !== false)
	{
		$pattern_callback = '%\[list(?:=([1a*]))?+\]((?:(?>.*?(?=\[list(?:=[1a*])?+\]|\[/list\]))|(?R))*)\[/list\]%is';
		$replace_callback = 'preparse_list_tag($matches[2], $matches[1], $errors)';
		$content = preg_replace_callback($pattern_callback, function($matches) use($replace_callback) {return $replace_callback;}, $content);
	}
...
$text);
	...
	$count = count($pattern);
	for ($i = 0; $i < $count; $i++) {
		$text = preg_replace_callback($pattern[$i], function($matches) use($i, $replace) {return $replace[$i];}, $text);
	}
...
function do_clickable($text, $unicode = FALSE)
{
	...
	if ($unicode)
	{
		$text= preg_replace_callback('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(https?|ftp|news){1}://([\p{Nd}\p{L}\-]+\.([\p{Nd}\p{L}\-]+\.)*[\p{Nd}\p{L}\-]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-]?)?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#iu', function($matches) {for($i=1;$i<=12;$i++){ $matches[$i]=isset($matches[$i])?$matches[$i]:'';} return stripslashes($matches[1].$matches[2].$matches[3].$matches[4]).handle_url_tag($matches[5].'://'.$matches[6], $matches[5].'://'.$matches[6], true).stripslashes($matches[4].$matches[10].$matches[11].$matches[12]);}, $text);
		$text = preg_replace_callback('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(www|ftp)\.(([\p{Nd}\p{L}\-]+\.)*[\p{Nd}\p{L}\-]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#iu', function($matches) {for($i=1;$i<=12;$i++){ $matches[$i]=isset($matches[$i])?$matches[$i]:'';} return stripslashes($matches[1].$matches[2].$matches[3].$matches[4]).handle_url_tag($matches[5].'.'.$matches[6], $matches[5].'.'.$matches[6], true).stripslashes($matches[4].$matches[10].$matches[11].$matches[12]);}, $text);
	}
	else
	{
		$text = preg_replace_callback('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(https?|ftp|news){1}://([\w\-]+\.([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-]?)?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#i', function($matches) {for($i=1;$i<=12;$i++){ $matches[$i]=isset($matches[$i])?$matches[$i]:'';} return stripslashes($matches[1].$matches[2].$matches[3].$matches[4]).handle_url_tag($matches[5].'://'.$matches[6], $matches[5].'://'.$matches[6], true).stripslashes($matches[4].$matches[10].$matches[11].$matches[12]);}, $text);
		$text = preg_replace_callback('#(?<=[\s\]\)])(<)?(\[)?(\()?([\'"]?)(www|ftp)\.(([\w\-]+\.)*[\w]+(:[0-9]+)?(/[^\s\[]*[^\s.,?!\[;:-])?)\4(?(3)(\)))(?(2)(\]))(?(1)(>))(?![^\s]*\[/(?:url|img)\])#i', function($matches) {for($i=1;$i<=12;$i++){ $matches[$i]=isset($matches[$i])?$matches[$i]:'';} return stripslashes($matches[1].$matches[2].$matches[3].$matches[4]).handle_url_tag($matches[5].'.'.$matches[6], $matches[5].'.'.$matches[6], true).stripslashes($matches[4].$matches[10].$matches[11].$matches[12]);}, $text);
	}
